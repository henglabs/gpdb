...
def genServerFsList():
    results = runQuery(
        "SELECT hostname, array_to_string(array_agg(fselocation), ' ') AS fs FROM pg_filespace_entry a , gp_segment_configuration b WHERE a.fsedbid = b.dbid GROUP BY hostname")
